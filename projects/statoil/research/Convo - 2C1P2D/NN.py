...
classifier = tf.estimator.Estimator(
    model_fn=cnn_model_fn, model_dir="/tmp/2C1P1D_1024_plot")
...
logging_hook = tf.train.LoggingTensorHook(
    tensors=tensors_to_log, every_n_iter=log_len)
    # Train the model
...
for i in range(100):
    ...
    eval_results_train = classifier.evaluate(input_fn=eval_input_fn_train)
    print(eval_results_train)
    current_epoch = current_epoch+log_len
    epoch.append(current_epoch)
    acc_test.append(eval_results["accuracy"])
    acc_train.append(eval_results_train["accuracy"])
    loss_train.append(eval_results_train["loss"])
    loss_test.append(eval_results["loss"])


plt.figure(1)
plt.subplot(211)
plt.plot(epoch,acc_test,'b')
plt.plot(epoch,acc_train,'r')

plt.subplot(212)
plt.plot(epoch,loss_test,'b')
