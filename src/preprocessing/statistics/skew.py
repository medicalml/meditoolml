...
def skew(x):
    ...
    .. _`Pearson's moment coefficient of skewness`: https://en.wikipedia.org/wiki/Skewness#Pearson.27s_moment_coefficient_of_skewness
    Equation:
        .. math::
            \\gamma_1 = E\\Bigg[\\bigg(\\frac{(X-\\mu)^3}{\\sigma}\\bigg)\\Bigg] = \\frac{\\mu^3}{\\sigma^3} =
            \\frac{E[(X - \\mu)^3]}{(E[(X - \\mu)^3])^{3/2}} = \\frac{\\kappa_3}{\\kappa_2^{3/2}}
        which can be rewritten as
        .. math::
            \\gamma_1 = \\frac{\\frac{1}{n} \\kahan_sum(X - \\mu)^3}{\\frac{1}{n} (\\kahan_sum(X - \\mu)^2)^{3/2}}
    Args:
    ...
    m2 = kahan_sum([pow((value - mean_x), 2) for value in x]) * (1 / n)
    m3 = kahan_sum([pow((value - mean_x), 3) for value in x]) * (1 / n)
