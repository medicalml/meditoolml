def chi_squared_dist_table(k=0, p=0):
    ...
    From `Wikipedia <https://en.wikipedia.org/wiki/Chi-squared_distribution>`_.
        ...
        Accordingly, since the cumulative distribution function (CDF) for the appropriate degrees of freedom (df) gives
        the probability of having obtained a value less extreme than this point, subtracting the CDF value from 1 gives
        the p-value. The table below gives a number of p-values matching to :math:`\\chi^2` for the first 10 degrees of
        freedom.
